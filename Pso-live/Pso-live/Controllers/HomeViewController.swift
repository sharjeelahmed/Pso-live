...
extension HomeViewController : UICollectionViewDataSource {
  ...
  func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    /*
     let cellData = collectionView.dequeueReusableCell(withReuseIdentifier: "DataCollectionViewCell", for: indexPath) as! DataCollectionViewCell
     return cellData
     */
    
    if indexPath.row == array_top.count {
      
      let cellData = collectionView.dequeueReusableCell(withReuseIdentifier: "DataCollectionViewCell", for: indexPath) as! DataCollectionViewCell
      
      cellData.label_saved.text = "\(homesaving.savings ?? "0")" //redCount
      cellData.label_offerused.text = "\(homesaving.redCount ?? "0")" //savings
      //cellData.label_smiles.text = "\(homesaving.smilesEarned!)"
